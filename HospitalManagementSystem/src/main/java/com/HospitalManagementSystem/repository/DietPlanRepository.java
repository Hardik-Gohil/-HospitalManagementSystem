...
public interface DietPlanRepository extends JpaRepository<DietPlan, Long>, JpaSpecificationExecutor<DietPlan> {
	...
	@Query("SELECT DISTINCT dietPlan.patient FROM DietPlan dietPlan INNER JOIN dietPlan.serviceMaster serviceMaster WHERE dietPlan.dietDate = :dietDate AND item LIKE :item AND serviceMaster.serviceMasterId IN (:serviceMasterIds)")
	...
	@Query("SELECT DISTINCT dietPlan.patient FROM DietPlan dietPlan WHERE dietPlan.dietDate = :dietDate AND item LIKE :item")
